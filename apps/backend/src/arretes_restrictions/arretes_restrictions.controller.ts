...
export class ArretesRestrictionsController {
  constructor(
    private readonly arretesRestrictionsService: ArretesRestrictionsService,
  ) {}

  @Get()
  @ApiOperation({
    summary: 'Récupérer les arrêtés de restrictions en vigueur par date',
  })
  @ApiResponse({
    status: 201,
  ...
  async situationByDepartement(
    @Query() query: ArretesRestrictionsQueryDto,
  ): Promise<ArreteRestrictionDto[]> {
    const { date, bassinVersant, region, departement } = query;
    return this.arretesRestrictionsService.getByDate(
      date,
      bassinVersant,
      region,
      departement,
