...
import {DataSource, FindManyOptions, FindOptionsWhere, In, IsNull, Not, Repository} from 'typeorm';
...
export class ZoneAlerteService {
    ...
    async getMaxUpdatedDate(currentUser: User): Promise<string> {
        const whereClause: FindOptionsWhere<ZoneAlerte> | null = !currentUser || currentUser.role === 'mte' ?
            {} : {
                departement: {
        ...
        const result = await this.zoneAlerteRepository.createQueryBuilder('zone_alerte')
