    ...
    for (
      ...
      try {
        await this.execPromise(
          `${path}/tippecanoe_program/bin/tippecanoe -Z3 -z14 -pg -ai -pn -f --drop-densest-as-needed -l zones_arretes_en_vigueur --read-parallel --detect-shared-borders --simplification=10 --output=${path}/${fileNameToSave}.pmtiles ${path}/${fileNameToSave}.geojson`,
        );
...
DELETE FROM zone_alerte_computed_historic 
  ...
  async computeGeoJson(date: Moment) {
    ...
    try {
      await this.execPromise(
        `${path}/tippecanoe_program/bin/tippecanoe -Z3 -z14 -pg -ai -pn -f --drop-densest-as-needed -l zones_arretes_en_vigueur --read-parallel --detect-shared-borders --simplification=10 --output=${path}/zones_arretes_en_vigueur_${date.format('YYYY-MM-DD')}.pmtiles ${path}/zones_arretes_en_vigueur_${date.format('YYYY-MM-DD')}.geojson`,
      );
